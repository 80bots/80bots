...
# Variables.
SERVICE=$(read_var SERVICE .env)
SERVICE=${SERVICE:-ngrok}
PUBLIC_URL=$(read_var PUBLIC_URL .env)
...
setup_env() {
  ...
  if [[ "$SERVICE" =~ ^(aws)$ ]]; then
    ...
    echo "APP_URL=${SCHEMA}://${PUBLIC_URL}:8080" >> .env
    echo "WEB_CLIENT_URL=${SCHEMA}://${PUBLIC_URL}:80" >> .env
    echo "WS_URL=${SCHEMA}://${PUBLIC_URL}:6001" >> .env

  elif [[ "$SERVICE" =~ ^(serveo)$ ]]; then
